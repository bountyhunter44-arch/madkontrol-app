# FILE: functions/admin/generateEgenkontrolFromRiskAnalysis.js
...
function inferAggregatedCategory(risk) {
    const process = normalizeText(risk.process);
    const hazard = normalizeText(risk.hazard);
    const controlType = normalizeText(risk.controlType);
    const name = normalizeText(risk.name);
    ...
    // Rengøring af udstyr (separat fra områder)
    if (
        (combined.includes("rengøring") || combined.includes("rengoering") || combined.includes("clean")) &&
        (combined.includes("udstyr") || combined.includes("equipment") || combined.includes("maskine"))
    ) {
        return "equipment_cleaning";
    }
...
function inferAggregatedDescription(aggregatedCategory, risks) {
    switch (aggregatedCategory) {
        ...
        case "hot_holding":
            return "Varmholdelse af retter ved min. 56°C. Ingen fast tidsgrænse i standardreglen.";
        case "general_checklist":
...
function buildAggregatedTemplate(locationId, aggregatedCategory, risks) {
    ...
    return {
        ...
        payload: {
            locationId,
            sourceRiskAnalysisIds: sourceRiskIds,
            aggregatedCategory,
            category: aggregatedCategory,  // Backward compatibility
            taskType,  // NEW: Stable task type identifier
            guideKey,  // NEW: Direct guide reference
            guideVersion,  // NEW: Guide version
            name,
            title: name,  // For consistency
            description,
            controlType,
            frequency,
            limitMin,
...
async function getExistingTemplatesByKeys(keys) {
    const found = new Map();
...
async function generateEgenkontrolFromRiskAnalysis({ locationId = null } = {}) {
    let query = db.collection("risk_analyses");
    ...
    console.log("Antal fundne risikoanalyser i collection:", riskSnap.size);

    if (riskSnap.empty) {
        return {
            ok: true,
            message: "Ingen risikoanalyser fundet i collectionen risk_analyses.",
            created: 0,
            updated: 0,
            skipped: 0,
            totalFound: 0,
    ...
    for (const doc of activeDocs) {
        try {
            ...
            const groupKey = `${locationId}__${aggregatedCategory}`;
    ...
    for (const [groupKey, risks] of risksByLocationAndCategory.entries()) {
        const [locationId, aggregatedCategory] = groupKey.split("__");
        templates.push(buildAggregatedTemplate(locationId, aggregatedCategory, risks));
    }
    ...
    const existing = await getExistingTemplatesByKeys(keys);
