...
```bash
node -e "const fs=require('fs'); const base=['public','modules','pos'].join('/'); for (const f of ['index.html','create-product.html','edit-product.html','delete-product.html','sales-report.html'].map(name => base + '/' + name)) { const html=fs.readFileSync(f,'utf8'); for (const s of html.matchAll(/<script(?![^>]*type=\"module\")[^>]*>([\\s\\S]*?)<\\/script>/g)) new Function(s[1]); if(!html.trim().endsWith('</html>')) throw new Error(f+' missing closing html'); console.log(f+' OK'); }"
```

## Manual smoke test

1. Åbn `/create-product.html`.
2. Opret en vare med navn og pris.
3. Åbn `/edit-product.html`.
