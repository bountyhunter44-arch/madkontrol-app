...
function looksLikeFirestoreId(value) {
  ...
  return Boolean(
    text &&
    (
      /^(company|location|onboarding)_[a-z0-9_-]+$/i.test(text) ||
      /^[a-z0-9]+(?:_[a-z0-9]+){2,}$/i.test(text)
    )
  );
}
...
function firstDisplayText(...values) {
  for (const value of values) {
    const normalized = clean(value);
    if (normalized && !looksLikeFirestoreId(normalized)) return normalized;
  }
  return "";
}
...
export async function resolvePrettyCompanyInfo({
  ...
  const companyName = firstDisplayText(
    company?.companyName,
    company?.name,
    company?.displayName,
    company?.prettyName,
  ...
  const locationName = firstDisplayText(
