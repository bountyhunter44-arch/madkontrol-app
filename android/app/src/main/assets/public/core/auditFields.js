...
export function resolveCurrentActor(user, profile = null) {
    if (!user) {
        ...
        const profileName = 
            ...
            profile?.liveUserProfile?.displayName ||
            profile?.liveProfile?.displayName ||
            ...
            prettyNameFromEmail(profile?.email || profile?.profile?.accountEmail || "") ||
            "System";
    ...
    // 1. users/{uid}.displayName / current profile displayName
    // 2. live_user_profiles/{uid}.displayName / nested profile displayName
    // 3. explicit person name fields
    // 4. user.displayName (if not company-like)
    // 5. pretty email prefix
    ...
    const profileCandidates = [
        ...
        ["profile.profile.displayName", profile?.profile?.displayName],
        ["profile.liveUserProfile.displayName", profile?.liveUserProfile?.displayName],
        ["profile.liveProfile.displayName", profile?.liveProfile?.displayName],
        ["profile.contactPersonName", profile?.contactPersonName],
        ["profile.profile.contactPersonName", profile?.profile?.contactPersonName],
        ["profile.profilname", profile?.profilname],
        ["profile.profile.profilname", profile?.profile?.profilname],
        ["profile.profilename", profile?.profilename],
        ["profile.profile.profilename", profile?.profile?.profilename]
    ];

    for (const [candidateSource, candidateValue] of profileCandidates) {
        const candidate = cleanNameCandidate(candidateValue);
    ...
    if (!name && user.email) {
        name = prettyNameFromEmail(user.email) || String(user.email).trim();
        source = "user.email_prefix";
    ...
    if (companyName && name.toLowerCase() === companyName) {
        ...
        warnings.push("actor_field_matches_company_name");
        name = prettyNameFromEmail(user.email) || "Ukendt bruger";
